<template>
  <div class="home">
    ...
    <el-row class="row2" display="margin-top:10px">      
        <el-input v-model="input" placeholder="请输入需要删除书号" style="display:inline-table; width: 30%; float:left"></el-input>
        <el-button  @click="delBook()" style="float:left; margin: 2px;">删除</el-button>
    </el-row>
    <el-row>
...
export default {
  ...
  methods: {
    ...
    delBook () {
      this.$http.get('http://127.0.0.1:8000/api/del_book?id=' + this.input2)
        .then((response) => {
          var res = JSON.parse(response.bodyText)
          if (res.error_num === 0) {
            this.showBooks()
